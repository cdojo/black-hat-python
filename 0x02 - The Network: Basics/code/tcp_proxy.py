...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
	...
	# Some server daemons will expect you to do this first (FTP sevre typically send a banner first, for example)
	if receive_first:
		# We then use our receive_from function, which we reuse for both sides of the comminication;
...
# 1 - Simply output the packet details with both their hexadecimal values and ASCII-printable characters.
...
# 2 - Is used both for receiving local and remote data, and we simply pass in the socket object to be used.
# by default, there is a two-second timeout set, which might be agressive if you are proxying traffic to other countries
